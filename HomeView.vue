<template>
    <div class="wrapper" id="wrapper" style="position: relative;">
        <div class="check-box" id="check-box" :style="selectedFont ? { fontFamily: selectedFont } : {}">
            <div style="position: relative;" id="check-box-print">
                ...
                <div v-if="check.accountHolderNameSecondary" class="account-holder-name" style="position: absolute; top: 65px; left: 60px">{{check.accountHolderNameSecondary}}</div>
                ...
                <div v-if="!printBlank" class="date-data" style="position: absolute; top: 80px; left: 850px">{{check.date}}</div>
                ...
                <div v-if="!printBlank" class="amount-data" style="position: absolute; top: 182px; left: 970px">{{formatMoney(check.amount)}}</div>
                <div v-if="!printBlank" class="pay-to-data" style="position: absolute; top: 180px; left: 180px">{{check.payTo}}</div>
                <div class="pay-to" style="position: absolute; top: 170px; left: 60px">
                    Pay to the <br>Order of <span class="payto-line"></span>
                </div>
                <div v-if="!printBlank" class="amount-line-data" ref="line" style="position: absolute; top: 240px; left: 100px">
                    ***
                ...
                <div v-if="!printBlank" class="memo-data" style="position: absolute; top: 367px; left: 120px">{{check.memo}}</div>
                <div class="memo" style="position: absolute; top: 390px; left: 60px">
                    Memo: ____________________________________
                </div>
                <div v-if="!printBlank" class="signature-data" style="position: absolute; top: 366px; left: 770px">{{check.signature}}</div>
        ...
        <div class="check-data" style="position: absolute; top: 450px">
            ...
            <button type="button" style="float: right;" class="btn btn-primary" @click="printCheck(false)">Print (Ctrl + P)</button>
            <button type="button" style="float: right; margin-right: 10px;" class="btn btn-outline-primary" @click="printCheck(true)">Print Blank</button>
            <form class="row g-3">
                <div class="col-md-4">
                    <label class="form-label">Check Font</label>
                    <select class="form-select" v-model="selectedFont">
                        ...
                        <option value="'Courier New', monospace">Classic Typewriter</option>
                        <option value="'Noto Serif', 'Times New Roman', serif">Noto Serif</option>
                        <option value="'Libre Baskerville', 'Times New Roman', serif">Libre Baskerville</option>
                ...
                <div class="col-md-6">
                    <label class="form-label">Account Holder Name (Additional)</label>
                    <input type="text" class="form-control" v-model="check.accountHolderNameSecondary">
                </div>
                <div class="col-md-6">
                </div>
                <div class="col-md-4">
...
function printCheck (blank = false) {
        printBlank.value = blank
...
function genNewCheck () {
    ...
    check.accountHolderNameSecondary = recentCheck?.accountHolderNameSecondary || ''
...
onMounted(() => {
    if (state.check) {
        ...
        check.accountHolderNameSecondary = state.check.accountHolderNameSecondary
